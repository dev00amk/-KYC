...
WITH funnel AS (
    SELECT
        ...
        AVG(ip_risk_score) AS avg_ip_risk_score,
        AVG(CASE WHEN engine_action = 'manual_review' THEN 1 ELSE 0 END) AS manual_review_rate,
        AVG(confirmed_fraud) AS confirmed_fraud_rate
...
audit_readiness AS (
    SELECT
        ...
        SUM(CASE WHEN commercial_po_box = 1 OR sanctions_hit = 1 THEN 1 ELSE 0 END) AS high_risk_attribute_count,
        AVG(CASE WHEN monitoring_trigger <> '' THEN 1 ELSE 0 END) AS post_approval_rereview_rate
    FROM kyc_applications
)
SELECT
    funnel.*,
    ...
    audit_readiness.post_approval_rereview_rate
FROM funnel
